        ...
        if not ingredients:
            ...
            for name, units in ingredients.items():
                if name:
                    ...
                    form.add_error(None, 'Такого ингредиента нет')
                amount = Amount(
                    units=units,
...
def del_purchase(request, recipe_id):
    recipe = get_object_or_404(Recipe, id=recipe_id)
    ToShop.objects.filter(user=request.user, recipe=recipe).delete()
    return redirect(shoping_list)
...
def print_shoping_list(request):
    ...
    recipes_to_shop = Recipe.objects.filter(
        recipe_to_shop__user=request.user)
